...
\begin{itemize}
    ...
    \[
    		\left(\sum_{i = 1}^n \alpha_i \ppsi_i^{\trp}\right) \cdot \left(\sum_{i = 1}^n \alpha_i \ppsi_i \right) = \sum_{i = 1}^n \alpha_i^2 \leq \sum_{i = 2}^n \alpha_i^2. 
    \]
    ...
    \[
    \|\DD^{1/2}\vecone_b\|_2^2 = (\DD^{1/2}\vecone_b)^{\trp} (\DD^{1/2}\vecone_b) = \left(\sum_{i=1}^n \beta_i \ppsi_i^\trp\right) \left(\sum_{i=1}^n \beta_i \ppsi_i\right) \leq \sum_{i=2}^n \beta_i^2.
    \]
\end{itemize}
...
\[
\hh_a = 1 + \sum_{a \sim b} \mathbb{P}[v_{t+1} = a \;| v_t = b]  \cdot \hh(b) = 1 + \sum_{a \sim b} \frac{\ww(a,b)}{\dd(b)}  \cdot \hh(b) =  1 + (\WW\vecone_a)^{\trp} \hh = 1 + \vecone_a^{\trp} \WW^{\trp} \hh.
\]
